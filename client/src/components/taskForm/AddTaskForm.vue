        ...
        >   
            ...
            <div class="row items-center q-mb-sm">
                ...
                <div class="q-gutter-sm ">
                    <q-radio keep-color v-model="status" val="pending" label="Pending" color="grey" disable/>
                    <q-radio keep-color v-model="status" val="working on" label="Working on" color="green-8" disable/>
                    <q-radio keep-color v-model="status" val="completed" label="Completed" color="blue-5" disable/>
                </div>
            </div>
...
const validate = async () => {
    ...
    try {
        let body = {
            ...
            owner: user.userDetail.id
            // status: status.value,
        }
        
        const response = await store.createTaskAction(body)
        if(response.res) {
            requestSuccess("Task creation done with success.")
            emit("success")
        } else {
